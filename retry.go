...
func (t *retryTransport) RoundTrip(req *http.Request) (*http.Response, error) {
	...
	for attempt := 0; ; attempt++ {
		...
		if err != nil {
			err = fmt.Errorf("forge-dashboard: round trip: %w", err)
		}
		...
		case <-req.Context().Done():
			...
			return resp, fmt.Errorf("forge-dashboard: %w", req.Context().Err())
		case <-timer.C:
		}
...
func rewind(req *http.Request) (*http.Request, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("forge-dashboard: rewind request body for retry: %w", err)
	}
...
func (*nonReplayableBodyError) Error() string {
	return "forge-dashboard: request body can't be replayed for a retry"
}
